...
SELECT
    ...
    (SELECT user_id FROM media WHERE media.id = likes.media_id) as u_id,
    (SELECT firstname FROM users WHERE users.id = u_id) AS user_name,
    (SELECT lastname FROM users WHERE users.id = u_id) AS user_lastname,
    (SELECT email FROM users WHERE users.id = u_id) AS user_email
FROM likes
GROUP BY u_id
